...
impl ParityCheckMatrix {
    ...
    pub fn new_inplace(mut bit_indices: Vec<usize>, 
    mut check_ranges: Vec<usize>,
    mut checks: Vec<Vec<usize>>, n_bits: usize) -> Self {

        bit_indices.truncate(0);
        check_ranges.truncate(0);
    ...
    pub fn circulant(indices: &Vec<usize>, l: usize) -> ParityCheckMatrix{
        ...
        for i in indices {
            ...
            for _ in prev_index .. *i {
                p_n = p_n.mul(&p); // for each jump in the indices we have to create the good P^n
            }
    ...
    pub fn circulant_right_better(indices: &Vec<usize>, l: usize) -> ParityCheckMatrix{

        let w = indices.len();
        let check_ranges:Vec<usize> = (0..l*w+1).step_by(w).collect();
        let mut bit_indices: Vec<usize> = Vec::with_capacity(l*w);
        ...
        for i in 0..l {
            ...
            (&mut bit_indices[i*w..(i+1)*w]).sort_unstable();
        }
    ...
    pub fn circulant_right_better_3(i_0: usize, i_1: usize, l: usize) -> ParityCheckMatrix{
        ...
        for i in 0..l {
            bit_indices.push((0+i)%l);
            bit_indices.push((i_0+i)%l);
            bit_indices.push((i_1+i)%l);
    ...
    pub fn circulant_right_better_5(i_0: usize, i_1: usize, i_2: usize, i_3: usize, l: usize) -> ParityCheckMatrix{
        ...
        for i in 0..l {
            ...
            bit_indices.push((i_0+i)%l);
            bit_indices.push((i_1+i)%l);
            bit_indices.push((i_2+i)%l);
            bit_indices.push((i_3+i)%l);
    ...
    pub fn circulant_right_better_5_inplace(work_pcm: ParityCheckMatrix, i_0: usize, i_1: usize, i_2: usize, i_3: usize, l: usize) -> ParityCheckMatrix{

        let w = 5;
        let mut check_ranges = work_pcm.check_ranges;
        check_ranges.clear();
        for i in (0..l*w+1).step_by(w) {
            check_ranges.push(i);
        }

        let mut bit_indices = work_pcm.bit_indices;
        bit_indices.clear();
    ...
    pub fn to_edge_list(&self) -> (Vec<usize>, Vec<usize>){
        let l = self.n_bits();

        let nb_checks = self.check_ranges().len()-1;
        let num_of_entries = self.check_ranges()[nb_checks-1];
        let mut list_v1: Vec<usize> = Vec::with_capacity(num_of_entries); //v1 will host the position of checks
        let mut list_v2: Vec<usize> = Vec::with_capacity(num_of_entries); //v2 will host the position ofqubits checks

        for ck in 0..nb_checks {
            if let Some(check) = self.check(ck) {
                ...
                for qubit in positions {
                    ...
                    list_v1.push(ck+1);
                    list_v2.push(qubit + nb_checks+1); // checks will have index 1 .. nb_checks
    ...
    pub fn rand_ldpc(r:usize, n: usize, w: usize) -> Self {
        
        let check_ranges:Vec<usize> = (0..r*w+1).step_by(w).collect();
        let mut bit_indices: Vec<usize> = Vec::with_capacity(r*w);
    ...
    pub fn rand_bicy(w: usize, n: usize ) -> Self {
        
        let mut checks: Vec<usize> = Vec::with_capacity(w);
        checks.push(0);
        ...
        Self::circulant_right_better(&checks, n)

    
    }

    pub fn rand_checks(w: usize, n: usize ) -> Vec<usize> {
    ...
    pub fn keep2(&self, bits: &[usize]) -> Self {
        let checks = self
            .checks_iter()
            .map(|check| {
                check
...
pub fn add_sparse_rows(a_row: &[usize], b_row:&[usize]) -> Vec<usize>  {

   // println!("{:?},{:?}", a_row, b_row);

    let mut accum = Vec::with_capacity(a_row.len()+b_row.len());
