...
impl ErasureDecoder{
    ...
    pub fn copy_pcm(&self) -> ParityCheckMatrix {
        self.checks.clone()
    }
}
...
impl<'a> Decoder for ErasureDecoder {
    ...
    fn decode(&self, error: &Self::Error) -> Self::Result {
        ...
        let erased_parity_check = self.checks.keep2(error);
    ...
    fn decode_mut(&self, error: &Self::Error, work_pcm: &mut ParityCheckMatrix) -> Self::Result {

        self.checks.keep_mut(work_pcm, error);
        if error.len() - work_pcm.rank() == 0 {
            ErasureResult::Succeed
        } else {
            ErasureResult::Failed
        }
