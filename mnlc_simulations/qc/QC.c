...
void randperm(int **rp, int n) {
   int i;
   for(i = 0; i < n; i++) (*rp)[i] = (i+1);

   for (i = 0; i < n; i++) {
      int j, t;
      j = rand() % (n-i) + i;
      t = (*rp)[j]; (*rp)[j] = (*rp)[i]; (*rp)[i] = t; // Swap i and j
   }
}
...
void Subd_Try(int current_crossings, int **Sixidx, int *Clabel, int *Dlabel, int **Cidx, int *pathedge, int N, int M, int *origedgelabel, int **incidente, int **deg, int *pdist, int *fi, int *fin, int vm, int d, int maxpdist, int *pidx, int *outputs, int orig_N, bool only_incident_check, int verbose ) { // IN ADVANCE:
...
// First, check for incident edge crossings.
   ...
   // Now check for subdivisions. These will occur if an edge crosses the same edge multiple times.
            ...
            for(j=0; j<qi1-qi; j++){
               ...
               // then subdvision incoming so reallocate (*Sixidx), (*Cidx), (*incidente), (*deg)
                  ...
                  *Cidx = (int *)realloc(*Cidx,(M+2)*sizeof(int));
                  for(k=0; k<maxpdist; k++){ //reset cc for use again
                     cc[k] = 0;
                  }
                  v2 = Clabel[cj];
                  if(verbose > 1)
                  ...
                  for(k=0; k<(*Cidx)[M-1]; k++){
                     if(k>=qi && k<=qi+ncnt1){ //these need checked
                        cnt3=cnt3+1;
                        Clabel[k] = newc1[cnt3];
                        Dlabel[k] = newd1[cnt3];
                     }
      ...
      free(pcnt); free(cc); free(newc1); free(newc2); free(newd1); free(newd2); free(pe); free(ze); free(de); free(qe); free(Mid); free(oe);
      ...
      if(copy_comb_ancestor_first[next-1] != 0){ 
         ...
         if( next == z){
            ...
            if(comb_ancestor_first[next-1] == 0){
               for(i=0; i<d; i++){  //need sizes of pathedge as input. check using pathedge matrix correctly
...
int Paths_Tree(int M, int *pidx, int *pathedge, int *paths, int *Sixidx, int d, int *fin, int *F, int *Fdir, int *Fidx, int *pdist, int maxpdist, int *pathsidx, int *comb_ancestor_sparse, int *comb_ancestor_first, int *comb_ancestor_next, int *eparent, int *vparent, int *comb_count, int *comb_ancestor_recent ) {
   ...
   free(chk); free(checked); free(stack); free(edgeentry); free(diststack); free(q);
...
void New_Clabel(int M, int Md, int *Sixidx_plan, int *Sixidx, int *Clabel, int *Dlabel, int *Cidx, int *elist, int *eidx, int *pathedge, int *paths, int *pidx, int *pathsidx, int *pdist, int maxpdist, int d, int *fin, int *fi, int *F, int *Fdir, int *Fidx, int vm, int *origedgelabel, int *origedgeorder, int newface, int *pathedgeonce, int treecheck, int maxdeadjm) {
   ...
   if(maxpdist > 0)
   {
      free(newedgelabel); free(newedgelabelun); free(checked);
   }
   ...
   free(endfaces); free(ctemp2); free(comb_ancestor_sparse); free(comb_ancestor_first); free(comb_ancestor_next); 
   free(eparent); free(vparent); free(comb_count); free(checked2); free(comb_ancestor_recent); free(order);
}
...
void Sp_Bigface(int *dvadj, int *Fidx, int source, int N, int *facelist, int facelistrows, int nav, int *dist, int *prev) {
   ...
   free(dest); free(lookdest); free(done); free(looknext); free(checkfaces);
}
...
int Get_Distances(int *dvadjm, int *Fidxm, int Ndm, int Mdm, int M_plan, int *Sixidx, int *dualel, int requiredimprovement, int d, int *fin, int *incidente, int *deg, bool *bad_edges, int current_crossings_best, int *deadjm, int mcnt, int *merged_face_labels, int max_F_index, int scheme, int bigface_depth, int maxdegfin, int maxdeadjm, int **pathedge, int *pidx, int *pathedgeonce, int **paths, int *pathsidx, int *pdist, int *delv, int *outputs, int bigface_fails) { 
   ...
   if(scheme == 1)
   {
      ...
      free(bad_face); free(faces_of_interest); free(a); free(prev); free(input_faces);
   }
   ...
   if(continue_looking)
   {
      ...
      for(k=1;k<=d;k++)
      {
         ...
         free(dvadj); free(Fidx); free(newv); free(done); free(looknext);
      }
      ...
      free(dist); free(prev); free(distsum);
   }
   else
   {
      used_bigface = 1;
...
void Merge_Dual(int Nd, int *Fidx, int *dualel, int M, bool *bad_edge, int *dvadj, int *ddeg, int *deadj, int num_bad_edges, int *bad_edge_labels, int *newd, int *newfidx, int *newel, int *pooledgemap, int *merged_face_labels, int *newe, int *outputs) { 
   ...
   free(degpool); free(face_checked); free(pooldeg); free(poole); free(poolf); free(poolidx); free(poolcurr); free(rel_chk);
}
...
void Quick_Cross_Main_Loop(int **incidente, int **Sixidx, int **Clabel, int **Dlabel, int **Cidx, int N, int M, int **deg, int current_crossings, int min_scheme, int bigface_depth, int stop_crossings, bool ignore_stop_crossings, int orig_N, int *outputs2, int verbose) {
   ...
   while(true)
   {
      ...
      if(debug==1) fprintf(stdout,"Running Planarise_Sixidx\n");
      int *Sixidx_plan = (int *)malloc((6*M + 6*(*Cidx)[M])*sizeof(int)); int *planvlabels = (int *)malloc(((*Cidx)[M])*sizeof(int)); int *elist = (int *)malloc((M + (*Cidx)[M])*sizeof(int)); 
      int *eidx = (int *)malloc((M+1)*sizeof(int)); int *origedgelabel = (int *)malloc((M+(*Cidx)[M])*sizeof(int)); int *origedgeorder = (int *)malloc((M+(*Cidx)[M])*sizeof(int));  int *incidente_plan = (int *)malloc((N)*sizeof(int)); 
      int newdim[2];
      ...
      if(debug==1) fprintf(stdout,"Finished Running Planarise_Sixidx\n");
      int N_plan = newdim[0]; int M_plan = newdim[1];
   
      if(debug==1) fprintf(stdout,"Running Faces\n");
      int *F = (int *)malloc((2*M_plan)*sizeof(int)); int *Fidx = (int *)malloc(((2*M_plan+5)/3)*sizeof(int)); int *Fdir = (int *)malloc((2*M_plan)*sizeof(int)); int *dvadj = (int *)malloc((2*M_plan)*sizeof(int)); 
      int *ddeg = (int *)malloc((2+M_plan-N_plan)*sizeof(int));  int *dualel = (int *)malloc((2*M_plan)*sizeof(int)); int *deadj = (int *)malloc((2*M_plan)*sizeof(int));
      int Nd = Faces(Sixidx_plan, M_plan, N_plan, NULL, 0, F, Fidx, Fdir, dvadj, ddeg, dualel, deadj);
      
   
      
      int *delv_best; int treecheck_best; int *pathedge_best; int *pidx_best; int *pathedgeonce_best; int *paths_best; int *pathsidx_best; int *pdist_best; 
      int *fin_best; int *fi_best; bool *bad_edges_best; int *dualel_best; int newface_best; int d_best; int *pooledgemap_best; int maxpdist_best; int maxmaxpaths_best; int maxdeadjm_best;
      ...
      for(i=1; i<=N; i++)
      {
         ...
         if(debug==1) fprintf(stdout,"Running Prep_Vars... ");
         int *fin; int *fi; int *bad_edge_labels; bool *bad_edges; int prepoutputs[4];
         ...
         int d = prepoutputs[0]; int requiredimprovement = prepoutputs[1]; int num_bad_edges = prepoutputs[2]; int maxdegfin = prepoutputs[3];
         
         if(debug==1) fprintf(stdout,"Running Merge_Dual... ");
         int *newdvadj = (int *)malloc((Fidx[Nd])*sizeof(int)); int *newfidx = (int *)malloc((Nd+1 + num_bad_edges)*sizeof(int)); int *newdualel = (int *)malloc((2*M_plan)*sizeof(int)); 
         int *pooledgemap = (int *)malloc((2*num_bad_edges)*sizeof(int)); int *merged_face_labels = (int *)malloc((2*num_bad_edges)*sizeof(int)); int *neweadj = (int *)malloc((Fidx[Nd])*sizeof(int)); 
         int mdoutputs[8];
         ...
         int newNd = mdoutputs[0]; int newMd = mdoutputs[1]; int maxFindex = mdoutputs[2]; int poolcnt = mdoutputs[3]; int mcnt = mdoutputs[4]; int cnt = mdoutputs[5]; int sumpooldeg = mdoutputs[6];
         int maxdeadjm = mdoutputs[7];
               
         if(debug==1) fprintf(stdout,"Running Get_Distances...\n");
         int *pathedge; int *paths; 
         int *pathedgeonce = (int *)malloc((2*maxdeadjm)*sizeof(int)); 
         int *pdist = (int *)malloc(d*sizeof(int)); int *pidx = (int *)malloc((d+1)*sizeof(int)); int *pathsidx = (int *)malloc((d+1)*sizeof(int)); int *delv = (int *)malloc(d*maxdegfin*sizeof(int)); int gdoutputs[9];
         ...
         int newcross = gdoutputs[0]; int treecheck = gdoutputs[1]; int newface = gdoutputs[2]; int maxpdist = gdoutputs[3]; int tempimprovementfound = gdoutputs[4]; int used_bigface = gdoutputs[5]; int maxmaxpaths = gdoutputs[6]; bigface_fails = gdoutputs[7]; min_scheme = gdoutputs[8];
                          
         if(debug==1) fprintf(stdout,"Finished Get_Distances...\n");        
         ...
         { 
            ...
            if(assigned_best)
            {
               free(delv_best); free(pathedge_best); free(pidx_best); free(pathedgeonce_best); free(paths_best); free(pathsidx_best); free(pdist_best); free(fin_best); free(fi_best); free(bad_edges_best); free(dualel_best); free(pooledgemap_best);
            }
            assigned_best = true;
         
            current_crossings_best = requiredimprovement + newcross; vm_best = vm; treecheck_best = treecheck; newface_best = newface; d_best = d; maxpdist_best = maxpdist; maxmaxpaths_best = maxmaxpaths; maxdeadjm_best = maxdeadjm;
            copy_array(delv, &delv_best, d*maxdegfin); 
            if(maxpdist == 0)
               copy_array(pathedge, &pathedge_best, d);
            else
               copy_array(pathedge, &pathedge_best, pidx[d]);
            copy_array(pidx, &pidx_best, d+1); copy_array(pathedgeonce, &pathedgeonce_best, 2*maxdeadjm);
            copy_array(paths, &paths_best, pathsidx[d]); copy_array(pathsidx, &pathsidx_best, d+1); copy_array(pdist, &pdist_best, d); copy_array(fin, &fin_best, d); copy_array(fi, &fi_best, d); 
            copy_array_bool(bad_edges, &bad_edges_best, M_plan); copy_array(newdualel, &dualel_best, 2*M_plan); copy_array(pooledgemap, &pooledgemap_best, 2*num_bad_edges);
         
            if(min_scheme < 3)
               breakinnerloop = true;
         } 
         
         if(debug==1) fprintf(stdout,"Freeing Variables\n");
         free(pathedge); free(paths); free(fin); free(fi); free(bad_edge_labels); free(bad_edges); free(newdvadj); 
         free(newfidx); free(newdualel); free(pooledgemap); free(merged_face_labels); free(neweadj); free(pathedgeonce);
         free(pdist); free(pidx); free(pathsidx); free(delv);
      ...
      if(improvementfound == 1)
      {
         ...
         if(debug==1) fprintf(stdout,"Running Find_Merge_Faces\n");
         ...
         if(debug==1) fprintf(stdout,"Up to New Clabel\n");
         ...
         N = stoutputs[0]; M = stoutputs[1]; current_crossings = stoutputs[2];
         free(newf_best); free(newfidx_best); free(newdir_best); free(rf_map_best);
      }
      else
      {
         if(verbose > 1)
      ...
      free(Sixidx_plan); free(planvlabels); free(elist); free(eidx); free(origedgelabel); free(origedgeorder); free(incidente_plan); 
      free(F); free(Fidx); free(Fdir); free(dvadj); free(ddeg); free(dualel); free(deadj); 
...
void Get_Distances_PE(int *dvadjm, int *Fidxm, int Ndm, int Mdm, int M_plan, int *Sixidx, int *dualel, int requiredimprovement, int d, int *fin, int *incidente, int *deg, int current_crossings_best, int *deadjm, int maxdegfin, int maxdeadjm, int **pathedge, int *pidx, int *pathedgeonce, int **paths, int *pathsidx, int *pdist, int *delv, int *outputs) {
   ...
   for(k=1;k<=d;k++)
   {
      ...
      dist[k-1 + (Nd-1)*d] = Nd;
      free(dvadj); free(Fidx); free(newv); free(done); 
      free(looknext);
   }
   for(i=1; i<=d; i++)
      dist[i-1 + (Nd-1)*d] = Mdm;
   ...
   *paths = (int *)malloc(pathsidx[d] * sizeof(int));
   ...
   free(checked); free(dist); free(prev); free(distsum);
...
void Embed_Planar_Subgraph(int **Sixidx, int **incidente, int **Clabel, int **Dlabel, int **Cidx, int N, int M, int **deg, int *vadj, int *vidx, int *eadj, int seed, int maxdeg, int *outputs, int verbose) {
   ...
   while(true){
      ...
      for(i=0; i<origN; i++){  //if we subdivide, new verts and edges are marked as inserted.
      ...
      int *pathedgeonce = (int *)malloc((2*maxdeadjm)*sizeof(int));
      int *pdist = (int *)malloc(d*sizeof(int)); int *pidx = (int *)malloc((d+1)*sizeof(int)); int *pathsidx = (int *)malloc((d+1)*sizeof(int)); int *delv = (int *)malloc(d*maxdegfin*sizeof(int)); int gdoutputs[7];
      ...
      free(Sixidx_plan); free(planvlabels); free(elist); free(eidx); free(origedgelabel); free(origedgeorder); free(incidente_plan); free(F); free(Fidx); free(Fdir);
      free(dvadj); free(ddeg); free(dualel); free(deadj); free(pathedgeonce); free(pdist); free(pidx); free(pathsidx); free(pathedge); free(paths); free(delv); free(bad_edges);
   
      if(verbose > 0)
         fprintf(stdout,"Planar Embedding: Added vertex: %d, current crossings: %d\n",vm,current_crossings);
   }

   free(dist); free(done); free(p); free(looknext); free(not_added_edges); free(checked); free(verts); free(fin); free(fi); free(fitemp);
...
void Embed_Circ(int **Clabel, int **Dlabel, int *Cidx, int *Sixidx, int N, int M, int maxdeg, int *vadj, int *eadj, int *vidx, int seed) {
   ...
   free(smap); free(cv2); free(cv); free(H); free(L); free(first); free(next); free(crscoord); free(crslabel); free(invsmap); free(crsdir);
}
...
void KKSL(long double *x, long double *y, int *Sixidx, int N, int *deg, int *vadj, int *vidx, int k, double l, int seed){
   ...
   free(contributionx_m); free(contributiony_m); free(Delta); free(par_x); free(par_y); free(T); free(L); free(K); free(stack); free(checked); free(smap);
...
void Get_Spring_Data(int *Sixidx, int *deg, int *vadj, int *vidx, int N, int M, int **Clabel, int **Dlabel, int *Cidx, int Clabel_max, int maxdeg, int seed, int providecoords, long double *x, long double *y){
   ...
   free(Clabeltemp); free(Dlabeltemp); free(t_values); free(tsort); free(tcheck); free(angles); free(vert_ind); free(vert_ang); free(next); free(prev); 
...
void UDFS(int v, int i, int *n, int *LP, int *Tcheck, int *T, int tcount, int *p, int *el, int *vidx, int *vadj, int *clst, int ccnt, int *eadj, int N, int *compidx, int compcnt, int *emap, int ecnt, int *elst, int *eidx, int *outputs, int M){
   ...
   for(w=vidx[v-1]+1; w<=vidx[v]; w++){
      ...
      if(n[q-1]==0){
         ...
         ccnt = outputs[2];
         ecnt = outputs[3];
         if( LP[q-1] >= n[v-1] ){ //then block completed
            if( T[tcount-1] != e){
               int *vcheck;
...
void Segment_Bipartite(int h, int wj, int M, int loz, int *Alefts, int *alfirst, int *alnext, int *Arights, int *arfirst, int *arnext, int *Elefts, int *elfirst, int *elnext, int *Erights, int *erfirst, int *ernext,  int *Ae, int Aecnt, int *Ee, int N, int *outputs){
   ...
   while(i>0 && (maxalb>loz || maxarb>loz) ){
      if(alfirst[i-1]>0 && maxalb>loz){
         if( arfirst[i-1]>0 && maxarb>loz){
            planar=0;
            outputs[1]=planar;
            free(aL); free(aR); free(eL); free(eR);
            return;         
         }  
         qv = alfirst[i-1];
         for(r=1; r<=N; r++){
   ...
   free(aL); free(aR); free(eL); free(eR); free(Alefts2); free(Arights2);
   free(Elefts2); free(Erights2); free(alfirst2); free(arfirst2); free(erfirst2);
   free(elfirst2); free(alnext2); free(arnext2); free(ernext2); free(elnext2);         
}
   ...
   for(j=scnt; j>=1; j--){
      ...
      for(r=2; r<=newdeg[wj-1]; r++){
         ...
         if(planar==0){
            ...
            outputs[2]=0;
            free(Spinepath); free(Alefts); free(Arights); free(Elefts); free(Erights); free(alfirst); free(arfirst); free(erfirst); free(elfirst); free(arnext); 
            free(alnext); free(ernext); free(elnext); 
            //free(nextfreear); 
            //free(nextfreeal); 
            free(Ae); free(Ee);
            return;
...
void Embedding_Given_Alpha(int *T, int *A, int *outputs, int *cyclic_adj, int *vidx, int e0, int t, int *alpha, int *newvadj, int *neweadj, int *newvidx, int M, int *deg, int *parent, int *backedge, int *newel, int N){
   ...
   for(j=scnt; j>=1; j--){
      ...
      opcnt=0;
      for(r=vidx[wj-1]+1; r<=vidx[wj]; r++){ //old undirected vidx here.
         opcnt++;
   ...
   outputs[0]=tcnt;
   outputs[1]=acnt;
   free(Spinepath); free(Tdash); free(Adash); free(AL); free(AR); free(op);
}
...
int Embedding_Given_Crossings(int N, int M, int *origclab, int *origcidx, int *elorig, int min_scheme, int stop_crossings, int bigface_depth, int verbose, int *finClab, int *finCidx){
  ...
  /***************************************************************************/
   ...
   int csize=0;
   for(i=1; i<=origM; i++){
      if(emap1[i-1]==0){ //then this edge was deleted so delete block
      ...
      for(j=origcidx[i-1]+1; j<=origcidx[i]; j++){
         if(emap1[origclab[j-1]-1]==0){//then this edge was deleted so remove single entry.
   ...
   for(z=1; z<=ctcnt; z++){
      ...
      for(i=1; i<=artcnt; i++){
         if(vinvmapcomp[artvert[i-1]-1]>Ndel1){//then this art vert came from a crossing and can be removed.
      ...
      if(outputs[2]==0){
         ...
         free(el); free(emap1); free(planvlabels); free(elistorig); free(eidxorig); free(planedges); free(origedgelabel); 
         free(Sixidxtotal); free(incidentetotal); free(deg); free(vadj); free(eadj); free(vidx); free(degidx); free(visited); 
         free(clst); free(ccidx); free(stack);
      
         free(LP); free(vinvmapcomp); free(einvmapcomp); free(vmapcomp); free(visited2); free(currdeg);  free(currvadj); free(curreadj); 
         free(currvidx); free(elcomp); free(currvadj2); free(curreadj2); free(currvidx2); free(elcomp2); free(Tcheck); 
         free(T); free(p); free(clst2); free(ccidx2); free(elst); free(eidx); free(artvert); free(currdeg2); free(vmapdfs); free(lowpt2); 
         free(backedge); free(newel); free(buckets); free(bucketcnt); free(p2); free(LP2); free (lowpt22); free(invvmapdfs); free(bucketfirst); 
         free(bucketnext); free(newvadj); free(neweadj); free(newvidx); free(newdeg); free(alpha);
      ...
      free(LP); free(vinvmapcomp); free(einvmapcomp); free(vmapcomp); free(visited2); free(currdeg);  free(currvadj); free(curreadj); 
      free(currvidx); free(elcomp); free(degidx); free(currvadj2); free(curreadj2); free(currvidx2); free(elcomp2); free(Tcheck); 
      free(T); free(p); free(clst2); free(ccidx2); free(elst); free(eidx); free(artvert); free(currdeg2); free(vmapdfs); free(lowpt2); 
      free(backedge); free(newel); free(buckets); free(bucketcnt); free(p2); free(LP2); free (lowpt22); free(invvmapdfs); free(bucketfirst); 
      free(bucketnext); free(newvadj); free(neweadj); free(newvidx); free(newdeg); free(alpha);
      free(incidente); free(cyclic_adj); free(Sixidx);
   ...
   for(z=1; z<=ctcnt; z++){
      ...
      for(i=1; i<=acompcnt; i++){
         ...
         free(currSixidx); free(vmapfinal); free(emapfin); free(einvmapfin); free(vinvmapfin);
         free(currclab); free(currdlab); free(currcidx); free(currincidente); free(currdego);   
      }
      free(visitedo); free(LPo); free(Tchecko); free(To); free(po); free(clst2o); free(ccidx2o);
      free(elsto); free(eidxo); free(artvertso);
   }
   int *fdn;
   ...
   free(origdlab); free(dego); free(vadjo); free(eadjo); free(vidxo); free(degidxo);
   
   free(vinvmap1); free(einvmap1); free(finCcnt);
   
   free(finCidx1); free(finClab1); free(fdn);
...
void Biconnected_Runner(int *el, int N, int M, int min_scheme, int embed_scheme, int bigface_depth, int seed, int verbose, int output_scheme, int stop_crossings, char *output_filename, int *finoutputs, int **finClab, int *finCidx, long double *x, long double *y) {
   ...
   for(z=1; z<=ctcnt; z++){
      ...
      if(acompcnt > 1){
         for(i=1; i<=acompcnt; i++){
            ...
            if(relcnt < 5){
               free(vmaptwo); free(currvadj); free(curreadj); free(currvidx); free(currdeg); free(f2); free(einvmaptwo); free(vinvmaptwo);
               continue; 
            }
            ...
            if(embed_scheme==3){
               ...
               if(Sixidx[2*Mcnt]==0){
                  free(vmaptwo); free(currvadj); free(curreadj); free(currvidx); free(currdeg); free(f2); free(Clabel);
                  free(Dlabel); free(Cidx); free(Sixidx); free(incidente); free(einvmaptwo); free(vinvmaptwo);
                  continue;
               }
               relcnt = outputs[0];
               Mcnt = outputs[1];
         ...
         if(ctcnt>1){ //then more than one component
            ...
            if(newN < 5){
               free(visited);
               free(relchk); free(vmaptwo); free(currdeg); free(currvadj); free(curreadj); free(currSixidx); free(currvidx); free(einvmaptwo); free(echeck); free(f2);
               continue; 
            }
            for(j=0; j<newN; j++){
               for(k=currvidx[j]; k<currvidx[j+1]; k++){
            ...
            if(embed_scheme == 3){
               ...
               if(currSixidx[2*newM]==0){
                  free(visited);
                  free(relchk); free(vmaptwo); free(currdeg); free(currvadj); free(curreadj); free(currSixidx); free(currvidx); free(einvmaptwo); free(echeck); free(f2);
                  free(incidente); free(Clabel); free(Dlabel); free(Cidx);
                  continue;
               }
               newN = outputs[0];
               newM = outputs[1];
            ...
            free(relchk); free(vmaptwo); free(currdeg); free(currvadj); free(curreadj); free(currSixidx); free(currvidx); free(f2); free(incidente); free(Clabel); free(Dlabel); free(Cidx); free(echeck); free(einvmaptwo);
         }
         else{  //single component, so just use initial info. 
               //deg 2 reductions 
            int t = 1;
            int v2;
            int f2cnt;
            ...
            if(N < 5){
               free(visited); free(LP); free(Tcheck); free(T); free(p); free(clst2); free(ccidx2); free(elst); free(eidx); free(f2);
               continue; 
            }
            int *Sixidx;
            ...
            if(embed_scheme==3){
               ...
               if(Sixidx[2*M]==0){
                  free(Sixidx); free(f2); free(Clabel); free(Dlabel); free(Cidx); free(incidente); free(vtwo);
                  continue;
               }
               N = outputs[0];
               M = outputs[1];
      ...
      free(visited); free(LP); free(Tcheck); free(T); free(p); free(clst2); free(ccidx2); free(elst); free(eidx);
   ...
   free(deg); free(vadj); free(degchk); free(ccidx); free(visited); free(stack); free(clst); free(finClab1); free(finCcnt); free(finCidx1); free(eadj); free(vinvmapdel1); free(einvmapdel1); free(fdn);
...
void Read_Graph(char *filename, int **elist, int *outputs) {
   ...
   *elist = (int *)malloc(2*M*sizeof(int));
   ...
   free(raw); free(next); free(first); free(entries);

   outputs[0] = N; 
   outputs[1] = M;
}

void Read_Graph_lib(const int *list_v1, const int *list_v2, int length, int **elist, int *outputs) {
   ...
   while(edge_list_index < length)
   {
   
      
   
      int v1 = list_v1[edge_list_index]; int v2 = list_v2[edge_list_index];
      //printf("v1:%d,v2:%d\n",v1,v2);
      edge_list_index++;
...
int get_cr(const int *list_v1, const int *list_v2, int length_edge_list) {
   ...
   bool use_edgelist_file = true; //the file has just been replaced by input vectors
   ...
   if(true)
   {
      ...
      {    
         ...
         {
            int outputs[2];
            Read_Graph_lib(list_v1, list_v2, length_edge_list ,&elist,outputs);
            N = outputs[0]; M = outputs[1];
         }
            ...
            if(dont_stop == true){
               if(Final_crossings2[0]<best_found){
                  ...
                  //then we compute a planarised graph and output that.
                     ...
                     free(planvlabels); free(elistorig); free(eidxorig); free(finelist);
         ...
         free(elist); free(finClabel); free(finCidx);     
      }   
   } 
   return Final_crossings2[0];
}
